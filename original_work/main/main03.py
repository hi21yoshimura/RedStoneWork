...
for i, row in tqdm(df_y_test.iterrows(), total=len(df_y_test)):
    ...
    errors_test_notuning.append(row.values - prediction_test) #np.absを削除
...
for i, row in tqdm(df_y_test.iterrows(), total=len(df_y_test)):
    ...
    errors_test_tuning.append(row.values - prediction_test) #np.absを削除
...
# baseline method
errors_baseline = y[:,T_train+1:] - y[:,T_train:-1] #np.absを削除
errors_baseline_ma = pd.DataFrame(errors_baseline.T).rolling(days_ma).mean()

# lower bound
errors_lower_bound = y[:,T_train+1:] - y_without_noise[:,T_train+1:] #np.absを削除
errors_lower_bound_ma = pd.DataFrame(errors_lower_bound.T).rolling(days_ma).mean()

# real data
#errors_real_data = np.array(y)# 指数表記を消さないと変換できない("e")
#errors_real_data_ma = pd.DataFrame(y).rolling(days_ma).mean()
...
for i_stock, name in enumerate(stock_names):
    ...
    #plt.plot(errors_real_data_ma[i_stock], "real_data")
    plt.xlabel("time")
